...
let gIndex = 1
const gUrl = "https://immgagllvk.execute-api.ap-northeast-2.amazonaws.com/Prod"
...
function ingredientListing() {
    $.ajax({
        type: "GET",
        url: `${gUrl}/ingredient-and-recipe`,
        data: {},
        success: function (response) {
...
function postRecipeInfo(status, info) {
    ...
    if (status == "filter") {
        ...
        $.ajax({
            type: "GET",
            url: `${gUrl}/recipe/search?recipe-search-name=${gRecipeSearchName}&sort=${gSorted[0]}`,
            success: function (response) {
                if (response['msg'] == 'success') {
                    gSorted = [];
    ...
    // 마이페이지 작성한 레시피를 눌렀을 경우
        ...
        $.ajax({
            type: "GET",
            url: `${gUrl}/recipe/search?mypage_id=${info}`,
            success: function (response) {
                gSorted = [];
                let idToAppend = status == "liked" ? "#recipe-liked-list" : "#recipe-liked-mypage-list"
                let idAlertNoLiked = status == "liked" ? "alert-no-liked" : "alert-no-liked-in-my-page"
